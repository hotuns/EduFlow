...
export const useUserStore = defineStore('user', {
  state: (): StoreState => ({
    ...
    questionScores:
      (electronStore.get('questionScores') as any) || QUESTION_SCORES,
    questionCounts:
      (electronStore.get('questionCounts') as any) || QUESTION_COUNTS,
  }),
  ...
  actions: {
    ...
    deleteUser(userName: string) {
      ...
      const userIndex = this.users.findIndex(
        (u) => u.name === userName && u.type === 'student'
      );
      if (userIndex >= 0) {
    ...
    saveExamResult(userName: string, score: number) {
      ...
      if (user) {
        ...
        user.examRecords = user.examRecords
          ? [...user.examRecords, newRecord]
          : [newRecord];
...
export function setupStoreSync() {
  ...
  const { users, currentUser, collapsed, questionScores, questionCounts } =
    storeToRefs(store);
