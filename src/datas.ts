...
export const getRandomQuestions = (questionBank: QuestionBank): Question[] => {
  ...
  const selectedQuestions = [
    ...
    ...shuffle([...questionBank.expand]).slice(0, questionCounts.expand),
...
class DataManager {
  ...
  async init() {
    ...
    try {
      ...
      const [
        ...
        this.loadExcel("expand.xls"), // 加载扩展题
      ]);

      // 转换Excel数据为题目格式
      this.questionBank.choice = this.transformChoiceQuestions(choiceData);
      this.questionBank.expand = this.transformExpandQuestions(expandData);
      this.questionBank.multiple =
        this.transformMultipleQuestions(multipleData);
      this.questionBank.judgment =
        this.transformJudgmentQuestions(judgmentData);
  ...
  private transformChoiceQuestions(data: any[]): Question[] {
    return data
      .filter((item) => item.title && item.optionA && item.optionB) // 确保必要字段存在
      .map((item, index) => ({
        id: index + 1,
        type: "choice",
  ...
  // 转换扩展题数据
  private transformExpandQuestions(data: any[]): Question[] {
    return data
      .filter((item) => item.title && item.optionA && item.optionB) // 确保必要字段存在
      .map((item, index) => ({
        id: index + 1,
  ...
  private transformJudgmentQuestions(data: any[]): Question[] {
    return data
      .filter((item) => item.title && item.answer) // 确保必要字段存在
      .map((item, index) => ({
        id: index + 1,
        type: "judgment",
        title: String(item.title).trim(),
  ...
  private transformEssayQuestions(data: any[]): Question[] {
    return data
      .filter((item) => item.title) // 确保必要字段存在
      .map((item, index) => ({
        id: index + 1,
        type: "essay",
  ...
  private transformMultipleQuestions(data: any[]): Question[] {
    return data
      .filter((item) => item.title && item.options) // 确保必要字段存在
      .map((item, index) => {
        // 解析选项字符串
