<template>
  <div class="w-full h-full flex flex-col space-y-4">
    ...
    <div class="w-full">
      <n-tabs type="line"
              animated>
        <n-tab-pane v-for="group in videoGroups"
                    :key="group.group"
                    :name="group.group"
                    :tab="group.group">
          <div class="flex flex-wrap gap-2">
            <n-button v-for="video in group.videos"
                      :key="video.id"
                      :type="video.id === currentVideo ? 'primary' : 'default'"
                      :disabled="!canPlayVideo(video.id)"
                      :class="{ 'opacity-60': !canPlayVideo(video.id) }"
    ...
    <div class="flex-1 flex flex-col space-y-4">
      ...
      <div class="flex items-center space-x-4 p-2">
        ...
        <n-dropdown :options="speedOptions"
        ...
        <!-- 修改进度条，添加可拖动功能 -->
        <div class="flex-1">
          <n-slider v-if="duration > 0"
                    :value="currentTime"
                    :max="duration"
                    :step="1"
                    :tooltip="false"
                    :disabled="!isVideoCompleted(currentVideo)"
...
const allVideosCompleted = computed(() => {
  ...
  return dataManager
    .getVideos()
    .every((video) => states.get(video.id)?.completed)
})
