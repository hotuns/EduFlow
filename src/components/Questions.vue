  ...
  <div class="questions-container">
    ...
    <div class="controls mb-4">
      <n-space justify="space-between"
               align="center">
        <n-select v-model:value="selectedType"
                  :options="typeOptions"
                  placeholder="选择题型"
                  style="width: 120px" />
        ...
        <n-space>
          ...
          <n-button size="small"
                    :type="currentIndex < filteredQuestions.length - 1 ? 'primary' : 'default'"
    ...
    <n-card v-if="currentQuestion"
      ...
      <div class="question-content mb-4">
        ...
        <template v-if="currentQuestion.type === 'choice' || currentQuestion.type === 'expand'">
          <n-radio-group v-model:value="userAnswer"
                         class="ml-4"
        ...
        <template v-else-if="currentQuestion.type === 'multiple'">
          <n-checkbox-group v-model:value="userAnswer"
                            class="ml-4">
            <n-space vertical>
        ...
        <template v-else-if="currentQuestion.type === 'judgment'">
          <n-radio-group v-model:value="userAnswer"
            ...
            <n-space>
              <n-radio value="true"
                       :disabled="answered && isCorrect">正确</n-radio>
              <n-radio value="false"
                       :disabled="answered && isCorrect">错误</n-radio>
            </n-space>
          </n-radio-group>
        </template>
      ...
      <div class="answer-section">
        <n-space justify="center">
          ...
          <n-button v-if="!answered && ['multiple', 'essay', 'fill'].includes(currentQuestion.type)"
                    type="primary"
                    @click="submitAnswer"
          ...
          <!-- 重新作答按钮，在答错时显示 -->
          <n-button v-if="answered && !isCorrect"
                    type="warning"
                    @click="resetCurrentAnswer">
            重新作答
          </n-button>
          <n-button :type="showAnswer ? 'default' : 'info'"
                    @click="toggleAnswer">
        ...
        <div v-if="showAnswer"
          ...
          <div v-else-if="currentQuestion.type === 'expand'"
               class="ml-4">
            {{ getAnswerLabel(currentQuestion) }}
          </div>
          ...
          <div v-else-if="currentQuestion.type === 'judgment'"
               class="ml-4">
